...
import {
  ...
  fetchAlbumPhotos,
  resetError,
} from "../redux/slices/userSlice"; // Corrected import
...
const UserDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (userId) {
      dispatch(fetchUserDetails(userId)); // Fetch user details
      dispatch(fetchUserAlbums(userId)); // Fetch albums by userId
    }
  ...
  useEffect(() => {
    if (userId && userAlbums.length > 0) {
      dispatch(fetchAlbumPhotos(userId)); // Fetch photos for albums
    }
  }, [dispatch, userId, userAlbums]);

  // Reset error when navigating away from the current user page
  useEffect(() => {
    return () => {
      dispatch(resetError()); // Reset error on unmount
    };
  }, [dispatch]);
